...
async function handleQuickDownload(url, customOptions = {}) {
  ...
  notify("Conversion en cours...", "Récupération des informations et encodage du média...");
  ...
  const title = customOptions.title || info.title || "Musique";
  const artist = customOptions.artist || info.artist || "";
  const format = customOptions.format || settings.defaultFormat || "mp3";
  const bitrate = customOptions.bitrate || (format === "mp4" ? "1080p" : "320k");
  const boost = customOptions.boost || settings.defaultBoost || "0";
  const youtubeCookies = await getBrowserCookies(targetUrl);

  // 2. Trigger Download API
  const downloadPayload = {
    url: customOptions.streamUrl || info.originalUrl || info.url || targetUrl,
    format,
    bitrate,
    quality: format === "mp4" ? bitrate : undefined,
    boost,
    editTitle: title,
    editArtist: artist,
